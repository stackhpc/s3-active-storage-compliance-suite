...
def main(workers, request_count):
    ...
    for N in SQUARE_DIMS:
        ...
        with ProcessPoolExecutor(max_workers=workers) as executor:
            _ = list(tqdm(executor.map(
                sum_without_proxy,
                *zip(*[(s3_fs, filename, request_data, X.sum()) for _ in range(request_count)])
            ), total=request_count))
        ...
        for name, url in PROXY_URLS.items():
            ...
            with ProcessPoolExecutor(max_workers=workers) as executor:
                _ = list(tqdm(executor.map(
                    sum_with_proxy, 
                    *zip(*[(url, request_data, X.sum()) for _ in range(request_count)])
                ), total=request_count))
...
if __name__ == '__main__':
    ...
    fig, axs = plt.subplots(figsize=(9, 3.5*len(workers_and_requests)), nrows=len(workers_and_requests))
    ...
    for ax, (workers, request_count) in zip(axs, workers_and_requests):    
        ...
        ax.set_ylabel(f"Time taken to handle {request_count} total requests\nfrom {workers} parallel workers (seconds)")
        ax.legend()
